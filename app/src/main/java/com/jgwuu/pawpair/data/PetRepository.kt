...
class PetRepository(private val petDao: PetDao, private val context: Context? = null) {
    ...
    suspend fun toggleSleep(petId: Int): Boolean {
        ...
        val updatedPet = pet.copy(
            ...
            finEstimado = 0L,
            dormirInicio = if (isSleeping) now else 0L,
            lastInteraction = now
        )
        petDao.updatePet(updatedPet)
    ...
    suspend fun applyDownloadedState(fetched: CoMascotaSyncService.FetchedState) {
        ...
        if (activePet != null) {
            ...
            if (prevTimestamp > 0 && fetched.syncTimestamp > prevTimestamp && context != null) {
                ...
                if (newLog != null) {
                    com.jgwuu.pawpair.util.NotificationHelper.showCoCareNotification(
                        context,
    ...
    suspend fun applyTimeDecayForAllPets() {
        ...
        for (pet in pets) {
            ...
            if (elapsedSeconds >= 12f) {
                ...
                var energyGainOrLoss = 0f
                var newDormirInicio = pet.dormirInicio
                if (pet.isSleeping) {
                    val realSleepMinutes = if (pet.dormirInicio > 0) ((now - pet.dormirInicio) / 60000f) else elapsedMinutes
                    energyGainOrLoss = realSleepMinutes * 2.0f
                    newDormirInicio = now
                } else {
                    energyGainOrLoss = -elapsedMinutes * 0.3f
                }
                ...
                if (stats != null && stats.notificationsEnabled && context != null) {
                    ...
                    if (hunger < 25f && pet.hunger >= 25f) {
                        com.jgwuu.pawpair.util.NotificationHelper.showPushNotification(
                            context,
