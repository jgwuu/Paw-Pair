...
object RealtimeSyncManager {
    ...
    fun startListening(code: String, onUpdate: (CoMascotaSyncService.FetchedState) -> Unit = {}) {
        ...
        petListener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                Log.d("RealtimeSyncManager", "onDataChange triggered at ${System.currentTimeMillis()} for code $activePetCode")
                try {
    ...
    fun pushUpdate(code: String, payload: JSONObject, context: android.content.Context? = null) {
        ...
        Log.d("RealtimeSyncManager", "pushUpdate start at $ts for code $code")
        val db = FirebaseManager.getDatabase() ?: return
        val cleanCode = code.trim().uppercase()
        if (cleanCode.isBlank()) return
        ...
        try {
            ...
            ref.updateChildren(updatesMap).addOnSuccessListener {
                Log.d("RealtimeSyncManager", "pushUpdate success at ${System.currentTimeMillis()} for code $cleanCode (took ${System.currentTimeMillis() - ts}ms)")
