...
object CoMascotaSyncService {
    ...
    fun uploadState(token: String, pet: PetEntity, stats: UserStatsEntity, logs: List<ActivityLogEntity>, inventory: List<InventoryEntity>, context: android.content.Context? = null, onResult: (Boolean) -> Unit = {}) {
        try {
            ...
            val dormirIni = pet.dormirInicio
            ...
            if (isFirebaseEnabled) {
                // Explicitly save to /pets/{idMascota}/codigoVinculacion
                val codeUrl = "${firebaseDbUrl.trimEnd('/')}/pets/$token/codigoVinculacion.json"
                val codeReq = Request.Builder().url(codeUrl).put("\"$linkCode\"".toRequestBody(JSON)).build()
                client.newCall(codeReq).enqueue(object : okhttp3.Callback {
                    override fun onFailure(call: okhttp3.Call, e: IOException) {}
                    override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {}
                })

                // If linkCode is different from token, save under /pets/{codigoVinculacion} as well
                if (linkCode != token) {
                    val altUrl = "${firebaseDbUrl.trimEnd('/')}/pets/$linkCode.json"
                    val altReq = Request.Builder().url(altUrl).put(payload.toString().toRequestBody(JSON)).build()
                    client.newCall(altReq).enqueue(object : okhttp3.Callback {
                        override fun onFailure(call: okhttp3.Call, e: IOException) {}
                        override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {}
                    })
                }

                // Shared state is now handled by RealtimeSyncManager
            }
        } catch (e: Exception) {
    ...
    fun parsePayloadToFetchedState(payload: JSONObject): FetchedState? {
        return try {
            ...
            val remoteDormirIni = petJson.optLong("dormirInicio", 0L)
            val now = System.currentTimeMillis()
