...
class PetRepository(private val petDao: PetDao, private val context: Context? = null) {
    ...
    suspend fun getLatestPet(): PetEntity? {
        val pet = petDao.getLatestPet() ?: return null
        if (pet.isSleeping && pet.dormirInicio > 0L) {
            val realtimeEnergy = pet.calculateRealtimeEnergy()
            if (realtimeEnergy != pet.energy) {
                val updated = pet.copy(energy = realtimeEnergy)
    ...
    suspend fun toggleSleep(petId: Int): Boolean {
        ...
        val dormirInicioTs = if (isSleeping) now else 0L
        
        val updatedPet = pet.copy(
            isSleeping = isSleeping,
            finEstimado = 0L,
            dormirInicio = dormirInicioTs,
            lastInteraction = now
        )
        petDao.updatePet(updatedPet)
    ...
    suspend fun applyTimeDecayForAllPets() {
        ...
        for (pet in pets) {
            ...
            if (elapsedSeconds >= 12f) {
                ...
                val energy = if (pet.isSleeping && pet.dormirInicio > 0L) {
                    pet.calculateRealtimeEnergy(now)
                } else {
                    (pet.energy + energyGainOrLoss).coerceIn(0f, 100f)
                }
