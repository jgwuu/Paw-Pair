...
object RealtimeSyncManager {
    ...
    fun startListening(code: String, onUpdate: (CoMascotaSyncService.FetchedState) -> Unit = {}) {
        ...
        try {
            ...
            Log.d("RealtimeSyncManager", "[SYNC-LOG][START-LISTENER] t=${System.currentTimeMillis()} ms | keepSynced(true) confirmado en /pets/$cleanCode")
        } catch (e: Exception) {
        ...
        petListener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                try {
                    val rcvT = System.currentTimeMillis()
                    Log.d("RealtimeSyncManager", "[SYNC-LOG][LISTENER-RCV] t=$rcvT ms | Recibida actualización en /pets/$activePetCode")
    ...
    fun fetchSingleState(code: String, onResult: (CoMascotaSyncService.FetchedState?) -> Unit) {
        ...
        if (cleanCode == activePetCode && petListener != null && _remoteState.value != null) {
            Log.d("RealtimeSyncManager", "[SYNC-LOG][FETCH-SKIP] t=${System.currentTimeMillis()} ms | Evitando get() competitivo, usando estado en tiempo real")
            onResult(_remoteState.value)
            return
        }
        val petRef = db.getReference("pets/$cleanCode")
    ...
    fun pushUpdate(code: String, payload: JSONObject, context: android.content.Context? = null) {
        ...
        Log.d("RealtimeSyncManager", "[SYNC-LOG][USER-ACTION -> WRITE-START] t=$startT ms | Iniciando escritura en /pets/$cleanCode")
        val ref = db.getReference("pets/$cleanCode")
        try {
            val updatesMap = flattenMapFromJsonObject(payload)
            ref.updateChildren(updatesMap).addOnSuccessListener {
                val dur = System.currentTimeMillis() - startT
                Log.d("RealtimeSyncManager", "[SYNC-LOG][WRITE-ACK] t=${System.currentTimeMillis()} ms | Escritura confirmada en Firebase (duración=$dur ms)")
