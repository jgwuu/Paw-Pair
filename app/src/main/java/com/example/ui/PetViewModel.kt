...
class PetViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        com.example.data.FirebaseManager.init(application)
        viewModelScope.launch {
            ...
            launch {
                com.example.data.RealtimeSyncManager.remoteState.collect { remote ->
                    if (remote != null) {
            ...
            launch {
                com.example.data.RealtimeSyncManager.latestAction.collect { action ->
                    if (action != null && action.timestamp > lastNotifiedTimestamp) {
                        lastNotifiedTimestamp = action.timestamp
                        val myUid = com.example.data.CoMascotaSyncService.getFirebaseUid(getApplication<Application>().applicationContext)
    ...
    fun pullSharedStateFromServer(onComplete: ((Boolean) -> Unit)? = null) {
        ...
        com.example.data.RealtimeSyncManager.fetchSingleState(code) { fetched ->
            if (fetched != null) {
        ...
        viewModelScope.launch {
            ...
            if (stats != null) {
                ...
                if (pet != null && currentCode != null && currentCode.length == 6 && currentCode != "XXXX-XXXX") {
                    com.example.data.CoMascotaSyncService.uploadState(
                        token = currentCode,
                        pet = pet,
                        stats = updatedStats,
    ...
    fun linkWithPartnerCode(code: String, onFinished: ((Boolean) -> Unit)? = null) {
        viewModelScope.launch {
            ...
            com.example.data.RealtimeSyncManager.joinPetAndFetch(cleanCode, myUid) { success, fetched ->
                viewModelScope.launch {
    ...
    fun triggerWidgetUpdate() {
        try {
            val context = getApplication<Application>().applicationContext
            val intent = android.content.Intent(context, com.example.widget.PetAppWidgetProvider::class.java).apply {
                action = android.appwidget.AppWidgetManager.ACTION_APPWIDGET_UPDATE
                val ids = android.appwidget.AppWidgetManager.getInstance(context).getAppWidgetIds(
                    android.content.ComponentName(context, com.example.widget.PetAppWidgetProvider::class.java)
                )
    ...
    fun releaseOrDeletePet(isOwner: Boolean, onResult: (Boolean, String) -> Unit) {
        viewModelScope.launch {
            ...
            val db = com.example.data.FirebaseManager.getDatabase()
            val uid = com.example.data.FirebaseManager.getCurrentUserId() ?: "anon"
